...
and safe to re-run; `--dry-run` prints the full plan and touches nothing. On an install that is already set up, a terminal run opens on a menu instead (add an SES region, AWS resources, base URLs, storage, social login, account email, a full walk-through, or start the stack).
