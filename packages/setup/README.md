...
On an install that is already set up, a terminal run opens on a menu instead of
walking every step: add an SES region, AWS resources, base URLs, storage, social
login, account email, a full walk-through, or start the stack.
```
