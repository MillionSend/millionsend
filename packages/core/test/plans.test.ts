...
import { effectivePlan, PLAN_GRACE_DAYS, raisesDailyLimit } from "../src/plans.js";
...
describe("raisesDailyLimit", () => {
  it("is true only when the move lifts the daily cap, unlimited counting as higher", () => {
    expect(raisesDailyLimit("free", "pro")).toBe(true);
    expect(raisesDailyLimit("free", "scale")).toBe(true);
    expect(raisesDailyLimit("pro", "scale")).toBe(true);
    expect(raisesDailyLimit("pro", "free")).toBe(false);
    expect(raisesDailyLimit("scale", "pro")).toBe(false);
    expect(raisesDailyLimit("scale", "scale")).toBe(false);
    expect(raisesDailyLimit("free", "free")).toBe(false);
  });
});
