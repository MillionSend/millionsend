...
base para ele. As diferenças notáveis são deliberadas e explícitas:

- Campos do Resend não suportados (ex.: `attachments`, `headers`
  customizados) são rejeitados com `422` em vez de descartados em silêncio.
- Contatos são globais ao time — endpoints de contato não recebem id de
  audience (veja [Contatos](/pt-BR/concepts/contacts)).
- Broadcasts suportam `canceled`, um status fora da união do Resend.
