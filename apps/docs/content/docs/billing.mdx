...
product's metadata, until they are moved, and carry no metered item, so
their sends past the included volume pass (the switch is on by default) but
bill nothing until one is added: switching overage off and back on in
Billing adds it, and so does a move up the ladder. The period counter starts
