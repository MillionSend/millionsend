...
4. **Switching rung** happens in the dashboard (`billing.changePlan`), and
   ...
   ladder has no metered item; the first time overage is turned on adds it.
...
passes the included volume, so there is nothing to report. Usage of a period
that already ended is stamped one second inside that period, where Stripe
invoices it (the invoice stays a draft for about an hour after the period
closes; rows older than 35 days can no longer be metered and are logged).
...
- **Portal configuration** is found by metadata and its features refreshed:
  ...
  cancel at period end with a cancellation reason collected; subscription
  updates are off (see [the flow](#the-flow)).
...
`overage_enabled` (default false) and `pending_rung`, and the `usage_periods`
...
product's metadata, until they are moved, and carry no metered item: the
first time overage is turned on adds it. The period counter starts empty:
sends accepted before the migration count against the day they were sent,
