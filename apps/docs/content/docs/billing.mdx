...
past it. Contacts, segments and topics are unlimited on every plan; sending
domains are capped at 3 on Free and 10 on Starter, unlimited above.
...
included volume depends on the **overage** switch in Billing, off by default:
...
- **Overage on**: reservations never fail. Sends past the included volume are
  reported to a Stripe meter and billed per 1,000 at the rung's rate on the
  next invoice (see [the overage cron](#the-overage-cron)).
...
configuration. A subscription carries the rung's price and, while overage is
on, the rung's metered price as a second item, whose id is stored in
`teams.stripe_overage_item_id` — that column is the overage switch every send
surface reads.
...
3. Stripe delivers `checkout.session.completed`, `customer.subscription.*`
   ...
   `current_period_end` and `stripe_overage_item_id`.
4. **Switching rung** happens in the dashboard (`billing.changePlan`): the
   subscription's items are updated with prorations — the plan item to the
   new rung's price, the overage item re-priced for a monthly rung or dropped
   for a daily one after its usage is reported — and the result applied at
   once; the webhook that follows re-applies the same state. The **overage**
   switch (`billing.setOverage`) adds or removes the metered item; off
   reports what is still unreported first.
5. **Manage billing** opens the Stripe Customer Portal for the payment
   method, invoices, tax id and cancel at period end. Plan changes are not
   offered there: Stripe's portal cannot update a subscription with more than
   one item, and one with overage on has two.
...
3. the legacy keys `millionsend_pro_monthly` → `pro_100k` and
   `millionsend_scale_monthly` → `scale_500k`;
4. the product's `metadata.millionsend_plan`, landing on that plan's first
   rung.
...
- `stripe_customer_id`, `stripe_subscription_id`, `current_period_start`,
  `current_period_end` and `stripe_overage_item_id` are stored alongside the
  plan. An overage item still on a legacy price is re-pointed to the rung's
  metered price as it is applied.

## The overage cron

`billing.overage` runs in the worker every 10 minutes. For every period row
with more sends past the included volume than the meter already knows about
(`accepted − included − reported_overage`), it sends one meter event per team
and period. The event's identifier is the counter it advances from
(`<team>:<period start>:<reported so far>`), and the row advances inside the
same transaction as the Stripe call: a report whose commit is lost is re-sent
under the same identifier and Stripe drops it as a duplicate, and a Stripe
failure leaves the row for the next run. Usage of a period that already ended
is stamped one second inside that period, where Stripe invoices it (the
invoice stays a draft for about an hour after the period closes; rows older
than 35 days can no longer be metered and are logged). The same report runs
before an overage item is removed, so nothing unbilled is lost.
...
- **Legacy prices** `millionsend_pro_monthly` and `millionsend_scale_monthly`
  are archived, not deleted: subscriptions still on them keep working through
  the legacy keys until each is moved; only new checkouts stop seeing them.
...
- **Portal configuration** is found by metadata and its features refreshed:
  invoice history, payment method, customer details including tax id, and
  cancel at period end; subscription updates are off (see
  [the flow](#the-flow)).
...
Migration `0035_pricing_ladder` adds the `starter` plan value, the
`plan_quota`, `current_period_start` and `stripe_overage_item_id` columns and
the `usage_periods` table. Existing `scale` teams map to Scale 500k
(`plan_quota` 500000) and `pro` teams to Pro 100k (100000);
`current_period_start` is backfilled as `current_period_end − 1 month`. Their
subscriptions stay on the legacy prices, resolved by the legacy lookup keys,
until they are moved. The period counter starts empty: sends accepted before
the migration count against the day they were sent, not against the period.
