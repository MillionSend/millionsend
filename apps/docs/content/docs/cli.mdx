...
| `--to-url <url>` | API URL of a self-hosted MillionSend instance. Unset, the target is MillionSend Cloud (`https://api.millionsend.com`), like the SDKs. |
...
| `MILLIONSEND_BASE_URL` | API URL of a self-hosted instance, same as `--to-url`. Unset means MillionSend Cloud. |
...
a terminal offers a choice between MillionSend Cloud and a self-hosted URL,
and a non-interactive run targets MillionSend Cloud, like the SDKs.
