...
| `.millionsend/migrate-report.md` | The same report as Markdown: counts, the checklist, the DNS records per domain, the id map (source topic and segment ids → MillionSend ids, for code that references them), the manual items. |
