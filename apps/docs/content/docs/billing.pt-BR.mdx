...
Stripe e podem cobrar excedente além dele. Contatos, segmentos e tópicos são
ilimitados em todos os planos; domínios de envio são limitados a 3 no Free e
10 no Starter, ilimitados acima.
...
no volume incluído depende da chave **excedente** em Cobrança, desligada por
padrão:
...
- **Excedente ligado**: as reservas nunca falham. Os envios além do volume
  incluído são reportados a um medidor do Stripe e cobrados por 1.000 na taxa
  do degrau, na próxima fatura (veja [o cron de excedente](#o-cron-de-excedente)).
...
preços por ambiente. A assinatura carrega o preço do degrau e, enquanto o
excedente está ligado, o preço medido do degrau como segundo item, cujo id
fica em `teams.stripe_overage_item_id` — essa coluna é a chave de excedente
que toda superfície de envio lê.
...
3. O Stripe entrega `checkout.session.completed`, `customer.subscription.*`
   ...
   `current_period_end` e `stripe_overage_item_id`.
4. **Trocar de degrau** acontece no painel (`billing.changePlan`): os itens
   da assinatura são atualizados com rateio — o item do plano para o preço do
   novo degrau, o item de excedente reprecificado para um degrau mensal ou
   removido para um diário depois de reportar o uso — e o resultado é
   aplicado na hora; o webhook que vem em seguida reaplica o mesmo estado. A
   chave de **excedente** (`billing.setOverage`) adiciona ou remove o item
   medido; desligar reporta antes o que ainda não foi reportado.
5. **Gerenciar cobrança** abre o Customer Portal do Stripe para forma de
   pagamento, faturas, id fiscal e cancelamento ao fim do período. Trocas de
   plano não são oferecidas lá: o portal do Stripe não consegue atualizar uma
   assinatura com mais de um item, e uma com excedente ligado tem dois.
...
3. as chaves legadas `millionsend_pro_monthly` → `pro_100k` e
   `millionsend_scale_monthly` → `scale_500k`;
4. o `metadata.millionsend_plan` do produto, caindo no primeiro degrau desse
   plano.
...
- `stripe_customer_id`, `stripe_subscription_id`, `current_period_start`,
  `current_period_end` e `stripe_overage_item_id` são guardados junto com o
  plano. Um item de excedente ainda num preço legado é reapontado para o
  preço medido do degrau ao ser aplicado.

## O cron de excedente

`billing.overage` roda no worker a cada 10 minutos. Para cada linha de
período com mais envios além do volume incluído do que o medidor já conhece
(`accepted − incluído − reported_overage`), envia um evento de medidor por
time e período. O identificador do evento é o contador de onde ele avança
(`<time>:<início do período>:<reportado até então>`), e a linha avança na
mesma transação da chamada ao Stripe: um relato cujo commit se perdeu é
reenviado com o mesmo identificador e o Stripe o descarta como duplicata, e
uma falha do Stripe deixa a linha para a próxima rodada. O uso de um período
que já terminou é carimbado um segundo dentro desse período, onde o Stripe o
fatura (a fatura fica em rascunho por cerca de uma hora após o fim do
período; linhas com mais de 35 dias não podem mais ser medidas e vão para o
log). O mesmo relato roda antes de um item de excedente ser removido, para
nada ficar sem cobrar.
...
- **Preços legados** `millionsend_pro_monthly` e `millionsend_scale_monthly`
  ...
  funcionando pelas chaves legadas até cada uma ser movida; só os novos
  checkouts deixam de vê-los.
...
- **Configuração do portal** é localizada por metadata e tem os recursos
  atualizados: histórico de faturas, forma de pagamento, dados do cliente
  incluindo id fiscal e cancelamento ao fim do período; atualização de
  assinatura fica desligada (veja [o fluxo](#o-fluxo)).
...
colunas `plan_quota`, `current_period_start` e `stripe_overage_item_id` e a
tabela `usage_periods`. Times `scale` existentes passam a Scale 500k
(`plan_quota` 500000) e times `pro` a Pro 100k (100000);
`current_period_start` é preenchido como `current_period_end − 1 mês`. As
assinaturas deles ficam nos preços legados, resolvidos pelas lookup keys
legadas, até serem movidas. O contador do período começa vazio: envios
