...
4. **Trocar de degrau** acontece no painel (`billing.changePlan`), e a
   ...
   anterior à escada não tem item medido; ligar o excedente pela primeira
   vez o adiciona.
...
reportar. O uso de um período que já terminou é carimbado um segundo dentro
desse período, onde o Stripe o fatura (a fatura fica em rascunho por cerca de
uma hora após o fim do período; linhas com mais de 35 dias não podem mais ser
medidas e vão para o log). O mesmo relato roda antes de a chave desligar,
...
- **Configuração do portal** é localizada por metadata e tem os recursos
  ...
  incluindo id fiscal e cancelamento ao fim do período com coleta do motivo;
  atualização de assinatura fica desligada (veja [o fluxo](#o-fluxo)).
...
`stripe_overage_item_id`, `overage_enabled` (padrão false) e `pending_rung`,
...
produto, até serem movidas, e não têm item medido: ligar o excedente pela
primeira vez o adiciona. O contador do período começa vazio: envios aceitos
antes da migração contam no dia em que saíram, não no período.
