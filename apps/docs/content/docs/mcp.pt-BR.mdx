...
- Um membro removido do time perde o acesso MCP imediatamente, mesmo antes de
  o token expirar.
- Seu papel no time vale aqui também: as ferramentas que gerenciam domínios e
  webhooks (e `get_webhook`, que retorna o segredo de assinatura) só são
  oferecidas a owners e admins, como no dashboard. Um **membro** com essas
  permissões concedidas ainda recebe as listagens somente leitura de domínios
  e webhooks. Em uma concessão **Todos os times** as ferramentas aparecem se
...
| `create_domain` | `domains:write` | **admin** Adiciona um domínio de envio e retorna os registros DNS. |
| `update_domain` | `domains:write` | **admin** Muda as configurações de rastreamento de um domínio. |
...
Nomes e propriedades de contatos, assuntos e corpos de e-mail e os nomes de
segmentos, tópicos, webhooks e domínios são todos escritos pelos seus
usuários finais ou por terceiros. O envelope permite ao agente separá-los da
