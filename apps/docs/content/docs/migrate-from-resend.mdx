...
| Domains | Created with return path and tracking settings, in the Resend domain's region — which must be the one region your MillionSend instance serves (MillionSend Cloud: `sa-east-1`); a domain in any other region is rejected with a 422 and listed in the report for you to add in the dashboard. On MillionSend Cloud, tracking toggles only carry over together with a tracking subdomain; without one the report lists them for you to set up in the dashboard. DNS records must be added again — see step 3. |
