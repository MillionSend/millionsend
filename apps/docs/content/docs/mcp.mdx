...
| `get_webhook` | `webhooks:write` | Get one webhook, including its signing secret. |
...
| `create_webhook` | `webhooks:write` | Create a webhook endpoint; the response includes the signing secret. |
| `update_webhook` | `webhooks:write` | Update a webhook's URL, events or status. |
| `delete_webhook` | `webhooks:write` | Delete a webhook endpoint. |
