...
- `properties` — a flat map of custom string values (`plan: "pro"`,
  ...
  removes that key. The stored map holds at most 100 keys.
...
Two MillionSend extensions make reading an audience cheap. `GET /contacts`
...
skipped. Each deletion is the same erasure as `DELETE /contacts/{id}`: the
address disappears from email history, event payloads and API logs. Resend
has no bulk deletion; this is a MillionSend extension, also exposed as the
`delete_contacts` MCP tool.
