...
- **Transactional sends** (`POST /emails`, SMTP relay): suppressed recipients
  ...
  the send is rejected with `422 all_recipients_suppressed` (message
  `All recipients are suppressed`). Recipients opted out of the send's
  `topic_id` count as suppressed for this rule.
