...
- `POST /suppressions` with `{ "email": "..." }` — blocks the address with
  origin `manual`. Idempotent: an address already suppressed for any reason
  keeps its entry and origin, and its existing id is returned.
- `POST /suppressions/batch/add` with `{ "emails": [...] }` and
  `POST /suppressions/batch/remove` with `{ "emails": [...] }` or
  `{ "ids": [...] }` — up to 1000 entries per call (Resend caps at 100).
  Add returns one id per distinct address in input order; remove lists only
  the rows actually removed.

Two MillionSend specifics: `origin: "unsubscribe"` is a superset value Resend
does not have (its SDK's type union lacks it), and an address whose personal
