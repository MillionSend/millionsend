...
- `POST /suppressions` with `{ "email": "...", "origin": "manual" }` — blocks
  the address. `origin` is optional (`bounce`, `complaint` or `manual`,
  default `manual`) and lets an import from another provider keep its bounce
  and complaint history. Idempotent: an address already suppressed for any
...
is a superset value Resend does not have (its SDK's type union lacks it; it
cannot be set on add), and an address whose personal
