...
| `email.prefetched` | O pixel foi buscado, ou um link seguido, antes de alguém abrir — pelo Mail Privacy Protection da Apple, pelo pré-carregamento do Gmail, por um scanner de segurança ou segundos após a entrega; `data.open.reason` ou `data.click.reason` diz qual (veja [precisão da taxa de abertura](/pt-BR/concepts/domains#precisão-da-taxa-de-abertura)). Opt-in: entregue só a endpoints que o listam, nunca a "todos os eventos". |
