...
| `quota.warning` | 80% da cota diária de hoje foi usada (uma vez por dia UTC, só na nuvem). | `{ used, limit, resets_at, dashboard_url }` |
| `quota.reached` | A cota de hoje foi usada; os envios passam até 10% a mais e depois estacionam até a meia-noite UTC. | igual ao anterior |
