...
- `POST /suppressions` com `{ "email": "...", "origin": "manual" }` — bloqueia
  o endereço. `origin` é opcional (`bounce`, `complaint` ou `manual`, padrão
  `manual`) e permite que uma importação de outro provedor preserve o
  histórico de bounces e reclamações. Idempotente: um endereço já suprimido
...
tipos do SDK dele não o inclui; não pode ser definido no add), e um
