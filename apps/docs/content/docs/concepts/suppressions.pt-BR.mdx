...
- **Envios transacionais** (`POST /emails`, relay SMTP): destinatários
  ...
  `422 all_recipients_suppressed` (mensagem `All recipients are suppressed`).
  Destinatários que optaram por sair do `topic_id` do envio contam como
  suprimidos para essa regra.
