...
- **A página de descadastro hospedada** lista os tópicos do time, então um
  destinatário pode sair de uma categoria e continuar inscrito nas outras —
  ou se descadastrar de tudo.

## API

`POST/GET/DELETE /topics` e `GET /topics/{id}` gerenciam tópicos; as escolhas
de um contato são gravadas com `PATCH /contacts/{id}/topics`, passando
entradas como `{ "id": "<topic-id>", "subscription": "opt_in" }`. Veja a
