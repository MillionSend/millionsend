...
| `email.prefetched` | The pixel was fetched, or a link followed, before anyone opened — by Apple Mail Privacy Protection, Gmail's prefetch, a security scanner, or within seconds of delivery; `data.open.reason` or `data.click.reason` says which (see [open-rate accuracy](/concepts/domains#open-rate-accuracy)). Opt-in: delivered only to endpoints that name it, never to "all events". |
