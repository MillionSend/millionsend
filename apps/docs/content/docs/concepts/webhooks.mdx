...
| `quota.warning` | 80% of today's daily cap is used (once per UTC day, cloud only). | `{ used, limit, resets_at, dashboard_url }` |
| `quota.reached` | Today's cap is used; sends pass up to 10% more, then park until midnight UTC. | same as above |
