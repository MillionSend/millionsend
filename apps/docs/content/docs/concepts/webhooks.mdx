...
| `email.opened` | A person loaded the tracking pixel (requires open tracking on the [domain](/concepts/domains)). `data.open` carries the fetch's `ipAddress`, `userAgent` and `timestamp`. |
| `email.clicked` | A rewritten link was clicked (requires click tracking). `data.click` carries `link`, `ipAddress`, `userAgent` and `timestamp`, as Resend's does. |
| `email.prefetched` | The pixel was fetched before anyone opened — by Apple Mail Privacy Protection, Gmail's prefetch, a security scanner, or within seconds of delivery; `data.open.reason` says which (see [open-rate accuracy](/concepts/domains#open-rate-accuracy)). Opt-in: delivered only to endpoints that name it, never to "all events". |
