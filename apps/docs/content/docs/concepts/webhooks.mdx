...
queue, drained in order at up to **20 requests per second**; a burst of
events waits in the queue rather than hitting the receiver all at once.
...
when its backlog is more than an hour old. The dashboard shows each
