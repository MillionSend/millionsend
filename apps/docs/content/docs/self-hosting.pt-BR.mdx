...
sem ajustes. Vem desligado por padrão: sem as variáveis `BACKUP_S3_*` o
contêiner imprime `backups disabled — set BACKUP_S3_* to enable` e sai com
código 0, inofensivo.

Ative definindo as quatro no `.env` (o bucket precisa existir antes; para o R2
os padrões `BACKUP_S3_PROVIDER=Cloudflare` e `BACKUP_S3_REGION=auto` já estão
corretos):
...
`BACKUP_RETENTION_DAYS` (padrão 14) são removidos. `BACKUP_S3_PREFIX` (padrão
`backups`) define o prefixo das chaves. Outros serviços compatíveis com S3
funcionam definindo `BACKUP_S3_PROVIDER` com o nome do provider no rclone
