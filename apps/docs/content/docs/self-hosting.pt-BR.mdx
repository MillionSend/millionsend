...
tocar em nada. Em uma instalação já configurada, uma execução no terminal abre um menu (adicionar uma região do SES, recursos da AWS, URLs base, storage, login social, e-mail da conta, o passo a passo completo ou iniciar a stack).
