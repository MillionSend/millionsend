...
| `NOTIFICATIONS_EMAIL_FROM` | Remetente das notificações de conta aos donos do time (cota diária quase ou totalmente usada, taxas de bounce/reclamação em risco ou pausadas) e dos e-mails de convite para o time, nos mesmos formatos de `AUTH_EMAIL_FROM`, que é o fallback. Sem nenhum dos dois, só os eventos de webhook saem e os convites ficam só por link. |
