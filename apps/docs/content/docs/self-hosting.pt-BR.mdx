...
roda `ghcr.io/millionsend/millionsend:latest`, que acompanha a `main` — todo
build ali passou pela suíte de testes antes. Para segurar uma versão, defina
...
segue o `BACKUP_CRON` (padrão `0 3 * * *`, UTC). Cada dump é um
