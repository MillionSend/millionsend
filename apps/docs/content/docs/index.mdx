...
It is a self-hostable alternative to Resend that sends through **your own AWS
SES account** — you keep SES's deliverability and pricing, and get a product
around it: a dashboard, an HTTP API, SDKs, webhooks, contacts, broadcasts, and
an SMTP relay.
...
A self-hosted deployment is two containers:
...
deliveries) flow back from SES through SNS — pushed to your host when it has a
public HTTPS URL, or long-polled from an SQS queue when it does not.

## Where to go next

- [Quickstart](/quickstart) — running locally and sending your first email in
  a few minutes.
