...
| Domínios | Criados com return path e configurações de rastreamento, na região do domínio no Resend — que precisa ser a única região atendida pela sua instância do MillionSend (MillionSend Cloud: `sa-east-1`); um domínio em qualquer outra região é recusado com 422 e listado no relatório para você adicionar no painel. No MillionSend Cloud, os toggles de rastreamento só vêm junto com um subdomínio de rastreamento; sem ele, o relatório os lista para você configurar no painel. Os registros DNS precisam ser adicionados de novo — veja o passo 3. |
