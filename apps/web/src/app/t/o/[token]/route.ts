...
/**
 * Public open-tracking endpoint. Verifies the signed open token and records a
 * unique open, then always returns the pixel — an invalid/tampered token gets
 * the same blank 1x1 GIF silently (never an error a mail client would surface).
...
export async function GET(_request: Request, ctx: { params: Promise<{ token: string }> }) {
  ...
  if (parsed) await recordEngagement(getDb(), parsed.emailId, "opened", enqueueWebhookDeliveries);
