...
/** Settled/open rows, each a second newer than the last so the settled order is fixed. */
async function delivery(
  status: "pending" | "success" | "failed" | "exhausted",
  opts: { nextAttemptAt?: Date; count?: number } = {},
) {
...
it("a backlog older than an hour mails once per UTC day", async () => {
